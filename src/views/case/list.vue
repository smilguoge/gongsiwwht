<template>
  <div class="app-container">
    ...
    <el-table
      ...
      style="width: 100%;">
      <el-table-column label="案例id" prop="id" align="center" width="">
        <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column label="案例名称" align="center" width="250px">
        <template slot-scope="scope">
          <span>
            {{ scope.row.title}}
          </span>
      ...
      <el-table-column label="分类id" align="center" width="">
        <template slot-scope="scope">
          <span>
      ...
      <el-table-column label="标签id" align="center" width="">
        <template slot-scope="scope">
          <span>
            {{ scope.row.tag_id}}
          </span>
      ...
      <el-table-column label="关键词" align="center" width="250px">
        <template slot-scope="scope">
          <span>
            {{ scope.row.keyword}}
          </span>
      ...
      <el-table-column label="描述" align="center" width="450px">
        <template slot-scope="scope">
          <span>
      ...
      <el-table-column label="点击量" align="center" width="">
        <template slot-scope="scope">
          <span>
            {{ scope.row.clicknum}}
          </span>
    ...
    <el-dialog  width="80%" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" :close-on-press-escape="dialogEsc" :close-on-click-modal="dialogEsc">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 800px; margin-left:50px;">
        <el-form-item label="案例首图" class="anli_img_top">
        ...
        <el-form-item label="点击量,浏览量" prop="">
         ...
         <el-form-item label="案例详情编辑" class="editor">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false, Reload()">取消</el-button>
...
<script>
import { getCaseList, postCase, deleteAdmin, getRolePlist } from '@/api/case'
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          postCase(this.temp).then(response => {
