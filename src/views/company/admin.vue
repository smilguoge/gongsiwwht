...
<script>
import { getCompanyAdminList, postCompanyAdmin, deleteCompanyAdmin } from '@/api/company'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getCompanyAdminList(this.listQuery).then(response => {
    ...
    handleModifyStatus(row, status) {
      row.status = status
      postCompanyAdmin(row).then(response => {
        this.$message({
          message: '操作成功',
          type: 'success'
        })
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.temp.company_id = this.company_id
          postCompanyAdmin(this.temp).then(response => {
            if (response) {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          postCompanyAdmin(tempData).then(response => {
